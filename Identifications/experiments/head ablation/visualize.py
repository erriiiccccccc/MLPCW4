...
def plot_ablation_heatmaps(df, config, metric="acc_drop", save=True):
    ...
    for idx, attn_type in enumerate(["temporal", "spatial"]):
        ...
        pivot = pivot.reindex(index=range(config.num_layers),
                              columns=range(config.num_heads), fill_value=0)
...
def plot_importance_heatmaps(df, config, save=True):
    fig, axes = plt.subplots(1, 2, figsize=(16, 8))
    ...
    for idx, attn_type in enumerate(["temporal", "spatial"]):
        ...
        ax.set_title(f"{attn_type.capitalize()} - Gradient Importance", fontsize=14)
        ax.set_xlabel("Head Index")
        ax.set_ylabel("Layer Index")

    plt.suptitle("Gradient-Based Head Importance", fontsize=16, y=1.02)
    plt.tight_layout()
...
def plot_cumulative_pruning(df, config, baseline_acc, save=True):
    ...
    if len(temporal_df) > 0:
        ...
        ax.plot(x_t, baseline_acc - temporal_df["acc_drop"].cumsum().values,
                label="Temporal only", linewidth=2, linestyle="--")
    ...
    if len(spatial_df) > 0:
        ...
        ax.plot(x_s, baseline_acc - spatial_df["acc_drop"].cumsum().values,
                label="Spatial only", linewidth=2, linestyle=":")
...
def plot_layer_importance(df, config, save=True):
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    if save:
        ...
        fig.savefig(path, dpi=150, bbox_inches="tight")
        print(f"Layer importance saved to {path}")
    plt.close(fig)


def plot_temporal_vs_spatial_scatter(df, config, save=True):
    ...
    for layer in layers:
        ax.scatter(spatial_means[layer], temporal_means[layer], s=100, zorder=5)
...
def generate_all_plots(ablation_df, importance_df, config, baseline_acc):
