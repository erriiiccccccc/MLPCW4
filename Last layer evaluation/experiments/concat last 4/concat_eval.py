...
print(f"Training probe (C={C_ADJUSTED})...")
...
shapley_path = os.path.join(PROBE_DIR, "summary", "shapley_weighted_results.json")
shap_acc = None
if os.path.exists(shapley_path):
    with open(shapley_path) as f:
        shap_acc = json.load(f).get("test_acc")

learned_path = os.path.join(PROBE_DIR, "summary", "learned_weighted_results.json")
learn_acc = None
if os.path.exists(learned_path):
    with open(learned_path) as f:
        learn_acc = json.load(f).get("test_acc")

print(f"\n{'Method':<30} {'Test Acc':>10}  {'Delta vs L11':>14}")
